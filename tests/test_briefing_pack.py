...
def test_export_writes_brief_and_leads_into_folder(
    ...
    """Default export creates a per-export folder with brief.md +
    ...
    assert Path(brief_path).name == "brief.md"
...
def test_export_legacy_paths_require_both(
    ...
    with pytest.raises(ValueError, match="both"):
        briefing_pack.export(out_path=str(tmp_path / "brief.md"))
...
def test_paired_export_cross_references_each_other(
    ...
    (`brief.md`, `leads.md`) — pairing is by folder, not by filename
    timestamp."""
    brief_path, leads_path = briefing_pack.export(
        out_dir=str(tmp_path / "20260510-1200"),
    )
    ...
    # Leads cites brief.md and notes the same.
    assert "Companion to: `brief.md` (in this folder," in leads
...
def test_export_records_brief_run(empty_findings, test_db_url, tmp_path):
    ...
    assert path == str(tmp_path / "20260510-1200" / "brief.md")
    assert top_n == 5
